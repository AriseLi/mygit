...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.backgroundColor = kWMColor(238, 238, 238);
...
- (UITableView *)classTab{
    if (_classTab == nil) {
        _classTab = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, 199, kWMScreenHeight-49-64)];
...
- (UITableView *)infoTab{
    if (_infoTab == nil) {
        _infoTab = [[UITableView alloc]initWithFrame:CGRectMake(200, 0, kWMScreenWidth-300-200, kWMScreenHeight-49-64)];
    ...
    if (tableView == self.classTab) {
        ...
        if (selectIndex == nil) {
            if (indexPath.section == 0 && indexPath.row == 0) {
                cell.titleLabel.textColor = kWMColor(254, 211, 54);
            }else{
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (tableView == self.classTab) {
        ...
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kWMScreenWidth-300 - 200, 60)];
        view.backgroundColor = [UIColor whiteColor];
